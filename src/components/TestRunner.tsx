...
export function TestRunner({ test, onBack, onComplete }: TestRunnerProps) {
    ...
    const { resultsMap, sectionScores, totalScore } = useMemo(() => {
        if (!submitted) return { resultsMap: {} as Record<string, boolean>, sectionScores: [] as { earned: number; total: number }[], totalScore: 0 };
        ...
        test.sections.forEach((section, si) => {
            ...
            if (section.type === 'multiple_choice') {
                ...
                section.segments.forEach(seg => {
                    ...
                    const ok = exactMatch(answers[key] || '', seg.answer, (seg as any).acceptAlso);
                    map[key] = ok;
            ...
            <CardContent sx={{ p: { xs: 2, sm: 3 } }}>
                <Box sx={{ mb: 2.5 }}>
                    ...
                    <Typography variant="caption" color="text.secondary">
                        {section.pointsNote}
                        {'instructions' in section && ` — ${(section as any).instructions}`}
                    </Typography>
                </Box>
...
function renderMultipleChoice(section: any, si: number, answers: any, setAnswer: any, submitted: boolean, resultsMap: any) {
    return (
        <Stack spacing={2}>
...
function renderFillFromBank(section: any, si: number, answers: any, setAnswer: any, submitted: boolean, resultsMap: any) {
...
function renderVerbTable(section: any, si: number, answers: any, setAnswer: any, submitted: boolean, resultsMap: any) {
...
function renderSentenceCompletion(section: any, si: number, answers: any, setAnswer: any, submitted: boolean, resultsMap: any) {
    return (
        <Stack spacing={2}>
...
function renderQuestionFormation(section: any, si: number, answers: any, setAnswer: any, submitted: boolean, resultsMap: any) {
    return (
        <Stack spacing={2.5}>
...
function renderPassageFill(section: any, si: number, answers: any, setAnswer: any, submitted: boolean, resultsMap: any) {
    return (
        <Box>
            {/* Passage */}
            <Paper
